...
export async function GET(
    ...
    try {
        ...
        const existingContest = await db
            .select()
            .from(contest)
            .where(eq(contest.id, parseInt(contestId)))
            .limit(1);
        ...
        const contestProblems = await db
            ...
            .where(eq(contestProblem.contestId, parseInt(contestId)))
...
export async function POST(
    ...
    try {
        ...
        const existingContestProblem = await db
            ...
            .from(contestProblem)
            .where(and(
                eq(contestProblem.contestId, parseInt(contestId)),
...
export async function DELETE(
    ...
    try {
        ...
        await db
            ...
            .where(and(
                eq(contestProblem.contestId, parseInt(contestId)),
                eq(contestProblem.problemId, parseInt(problemId))   
            ));
