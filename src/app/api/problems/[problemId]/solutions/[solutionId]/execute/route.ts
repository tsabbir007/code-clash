...
export async function POST(
    ...
    try {
        ...
        const solutions = await db
            ...
            .where(
                and(
                    eq(solution.id, parseInt(solutionId)),
                    eq(solution.problemId, parseInt(problemId))
                )
            );
        ...
        for (let i = 0; i < testCases.length; i++) {
            ...
            try {
                ...
                // Process the result
                let verdict = 'AC';
                let cpuTime = Math.round(parseFloat(result.time || '0') * 1000);
                let memoryUsage = Math.round(result.memory || 0 / 1024);
                let error = null;
        ...
        const overallVerdict = results.some(r => r.verdict === 'RE') ? 'RE' :
            results.some(r => r.verdict === 'WA') ? 'WA' :
            results.some(r => r.verdict === 'TLE') ? 'TLE' :
            results.some(r => r.verdict === 'MLE') ? 'MLE' :
            results.some(r => r.verdict === 'CE') ? 'CE' : 'AC';
