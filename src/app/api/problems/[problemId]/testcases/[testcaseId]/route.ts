...
export async function PUT(
    request: NextRequest,
    { params }: { params: Promise<{ problemId: string; testcaseId: string }> }
) {
    try {
        const { problemId, testcaseId } = await params;
        ...
        console.log('Updating test case:', testcaseId, 'for problem:', problemId, 'by user:', session.user.id);
        ...
        const updatedTestCases = await db
            ...
            .where(
                and(
                    eq(testCase.id, parseInt(testcaseId)),
                    eq(testCase.problemId, parseInt(problemId))
                )
            )
            .returning();
...
export async function DELETE(
    ...
    try {
        ...
        console.log('Deleting test case:', testcaseId, 'for problem:', problemId, 'by user:', session.user.id);
