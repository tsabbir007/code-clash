...
export async function GET(
    ...
    try {
        ...
        const solvedProblems = submissionsResult.filter(sub =>
            (sub.verdict === 'Accepted' || sub.verdict === 'AC') && sub.verdict !== 'Pending' && sub.verdict !== null
        ).length;
        ...
        // Calculate standings
        const standingsResult = await calculateContestStandings(contestIdNum, problemsResult, contestData.startTime);
    ...
    try {
        ...
        for (const participant of participantsResult) {
            ...
            const problemResults = problems.map(problem => {
                ...
                const bestScore = problemSubmissions.length > 0 ? Math.max(...problemSubmissions.map(s => s.score || 0)) : 0;
                const firstSolveTime = isSolved ?
                    problemSubmissions.find(s => (s.verdict === 'Accepted' || s.verdict === 'AC'))?.submissionTime : undefined;
            ...
            const penalty = problemResults.reduce((sum, pr) => {
                ...
                if (pr.isSolved && pr.firstSolveTime) {
                    ...
                    const problemSubmissions = userSubmissions.filter(sub => sub.problemId === pr.problemId);

                    // Find submissions before and including the first accepted submission
                    const firstAcceptedIndex = problemSubmissions.findIndex(sub => sub.verdict === 'Accepted' || sub.verdict === 'AC');
